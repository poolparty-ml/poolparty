...
let rec wait_for_job connection_manager_pid ref item =
  ...
  | Messages.CheckOut requester_pid ->
    ...
    send requester_pid (Messages.HolderMessage (holder_pid, ref, item))
  | _ -> error (fun f -> f "Unknown message"));

  (*
  * loop the process over and over again
  *)
  wait_for_job connection_manager_pid ref item
...
let holder_start_link { connection_manager_pid; ref; item } =
  let child_pid =
    spawn_link (fun () -> wait_for_job connection_manager_pid ref item)
  in
...
let new_holder connection_manager_pid ref item =
  let state = { connection_manager_pid; ref; item } in
